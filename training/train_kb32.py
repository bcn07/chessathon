"""Train the 768 -> H -> 1 NNUE with 32 king buckets (a copy of ``train.py``; see it for the
rest of the docstring). The only difference is that ``decode_batch`` can index
``encoding.KING_BUCKET_32`` (one bucket per (rank, file a-d) king square, 24576 input rows) when
``--king-buckets 32`` is given; every other value behaves exactly as ``train.py``.
...
import numba
from encoding import KING_BUCKET, KING_BUCKET_32, MAX_PIECES, NUM_FEATURES
    ...
    for i in range(n):
        ...
        if king_buckets > 1:
            ...
            if (king & 7) >= 4:
                mirror = np.int64(7)
            if king_buckets == 32:
                offset = KING_BUCKET_32[king ^ mirror] * NUM_FEATURES
            else:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="1 = plain 768 inputs; 8 = mirror the board to the king's half and offset the "
             ...
             "32 = one bucket per (rank, file a-d) king square (encoding.KING_BUCKET_32)",
    )
    parser.add_argument(
        "--wdl",
        type=float,
