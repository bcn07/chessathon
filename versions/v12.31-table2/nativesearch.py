...
def _mop_up(state: np.ndarray, winner: int, loser: int) -> int:
    ...
    if material == 0 and minor_count < 2:
        return 0
    # ``lsb_square`` returns uint8, and ``king >> 3`` widens it to uint64; the rank difference
    # below is therefore an unsigned subtraction that wraps, ``abs`` is a no-op on it, and the
    # whole function types as float64 -- which then propagated through ``evaluate_state_into``
    # into ``_quiesce`` and ``_negamax`` and made numba compile each of them seven times
    # (alpha x beta x ply over int64/float64/Literal). Casting the two squares to int64 pins the
    # type. The rank term keeps its missing ``abs`` on purpose: this rewrite is search-identical,
    # and restoring the absolute value changes the mop-up score in half of all king placements
    # (a real bug -- see the report -- but one that has to be tested for Elo on its own).
    winner_king = np.int64(fb.lsb_square(state[winner_offset + fb.KING]))
    loser_king = np.int64(fb.lsb_square(state[loser * 6 + fb.KING]))
    loser_file, loser_rank = loser_king & 7, loser_king >> 3
    edge_distance = min(loser_file, 7 - loser_file, loser_rank, 7 - loser_rank)
    king_distance = abs((winner_king & 7) - loser_file) + ((winner_king >> 3) - loser_rank)
...
TT_BITS = 24  # 16 M entries (~320 MB): a 3-6 s move overfilled 2^22 two to three times
...
def search_root(
    ...
    best_move = np.uint32(0)
    legal_count = 0
    # A literal 1 for the children's ply makes numba compile a second Literal[int](1)
    # specialisation of the whole of _negamax (and of _quiesce below it) just for the root's
    # children; np.int64 keeps the value and drops the specialisation.
    child_ply = np.int64(1)
    for index in range(count):
        _pick_next(moves, scores, index, count)
        move = moves[index]
        undo = fb.make_move(state, move)
...
class Searcher:
    ...
    def think(
        ...
        for depth in range(1, max_depth + 1):
            ...
            if depth >= 4 and abs(previous_score) < MATE_BOUND:
                alpha = max(-INF, previous_score - ASPIRATION_WINDOW)
                beta = min(INF, previous_score + ASPIRATION_WINDOW)
            else:
                # A window around a mate score cannot bracket a shorter mate found next
                # iteration; search those full-width instead of paying for a re-search.
                alpha, beta = -INF, INF
            ...
            depth_reached = depth
            # A mate score is only trustworthy once the search is deep enough to have walked the
            # whole mating line. Stopping at the first mate score lets a transposition-table hit
            # end the iteration at depth 1 in zero nodes; the move it returns is then whatever
            # transposes into a stored mate, so the mate distance oscillates (99981, 99985,
            # 99983 ...) and the engine shuffles until a repetition or the fifty-move rule. Basic
            # KQ v K and KR v K are drawn that way. Keep iterating until the depth covers the
            # announced distance to mate, which forces the search to prove and shorten it.
