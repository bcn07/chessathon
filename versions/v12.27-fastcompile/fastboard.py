...
# Slider attacks by "kindergarten" table lookups: the occupancy of one line is gathered into a
# six-bit key and LINE_ATTACKS gives the attack set. Two lookups replace a scan of four rays,
# which move generation, check detection, SEE and the evaluation all pay for on every slider.
...
def rook_attacks(square: int, occupied: np.uint64) -> np.uint64:
    """Orthogonal attacks, including the first occupied square on each ray."""
    rank_key = int(
...
def bishop_attacks(square: int, occupied: np.uint64) -> np.uint64:
    """Diagonal attacks, including the first occupied square on each ray."""
...
@numba.njit
def rook_attacks_scan(square: int, occupied: np.uint64) -> np.uint64:
    """The original ray scan, kept as the reference the lookups are checked against."""
    attacks = U64_ZERO
    file_index, rank_index = square & 7, square >> 3
...
@numba.njit
def bishop_attacks_scan(square: int, occupied: np.uint64) -> np.uint64:
    """The original ray scan, kept as the reference the lookups are checked against."""
    attacks = U64_ZERO
    file_index, rank_index = square & 7, square >> 3
...
# Numba specialises a jitted function on the integer *literals* at a call site. The castling
# block below tests ten fixed squares, which made ``is_square_attacked`` -- and the slider lookups
# it calls -- compile eleven times instead of once. Reading the square out of this table keeps the
# value and drops the specialisation.
...
def generate_pseudo_into(state: np.ndarray, moves: np.ndarray) -> int:
    ...
    if color == WHITE and state[WK] & (U64_ONE << np.uint64(4)):
        if (
            ...
            and not is_square_attacked(state, SQ[4], opponent)
            and not is_square_attacked(state, SQ[5], opponent)
            and not is_square_attacked(state, SQ[6], opponent)
        ):
            moves[count] = encode_move(SQ[4], SQ[6], 0, FLAG_CASTLE)
            count += 1
        if (
            ...
            and not is_square_attacked(state, SQ[4], opponent)
            and not is_square_attacked(state, SQ[3], opponent)
            and not is_square_attacked(state, SQ[2], opponent)
        ):
            moves[count] = encode_move(SQ[4], SQ[2], 0, FLAG_CASTLE)
            count += 1
        ...
        if (
            ...
            and not is_square_attacked(state, SQ[60], opponent)
            and not is_square_attacked(state, SQ[61], opponent)
            and not is_square_attacked(state, SQ[62], opponent)
        ):
            moves[count] = encode_move(SQ[60], SQ[62], 0, FLAG_CASTLE)
            count += 1
        if (
            ...
            and not is_square_attacked(state, SQ[60], opponent)
            and not is_square_attacked(state, SQ[59], opponent)
            and not is_square_attacked(state, SQ[58], opponent)
        ):
            moves[count] = encode_move(SQ[60], SQ[58], 0, FLAG_CASTLE)
            count += 1
    return count
